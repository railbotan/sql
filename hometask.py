...
df['skills'] = df['skills'].apply(remove_html_tags)
df['otherInfo'] = df['otherInfo'].apply(remove_html_tags)
...
cursor.execute('CREATE TABLE education'
               '(id INTEGER PRIMARY KEY AUTOINCREMENT, edu_level TEXT)')
cursor.execute('INSERT INTO education(edu_level) '
               'SELECT DISTINCT educationType FROM works '
...
cursor.execute('UPDATE works '
               ...
               '(SELECT id FROM education WHERE edu_level = works.educationType)')
cursor.execute('ALTER TABLE works '
